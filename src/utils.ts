...
export function trackNft(transfer: LogDescription, timestamp: number) {
  ...
  if (!nftMap.has(tokenId)) {
    nftMap.set(tokenId, {
      salesHistory: [{ buyer: buyer, seller: seller, saleAt: timestamp }],
      salesMetadata: {
        [buyer]: { count: 1, firstSaleAt: timestamp, findingSent: 1 },
      },
    });
...
export function trackNftSale(tokenId: string, transfer: LogDescription, timestamp: number) {
  ...
  nftReport.salesHistory.push({
    buyer: buyer,
    seller: seller,
    saleAt: timestamp,
  });
  if (!nftReport.salesMetadata[buyer]) {
    nftReport.salesMetadata[buyer] = {
      count: 1,
